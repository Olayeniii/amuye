...
from .acp import AcpRiskAssessmentProvider, NodeAcpRiskClient
...
def run(protocol: str, memory_db: Path, output: Path) -> dict:
    request = replace(
        ...
        clientId="checkpoint4-live-acp-demo",
    )
    ...
    provider = AcpRiskAssessmentProvider(
        NodeAcpRiskClient(),
        ProtocolAssessmentProvider(),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Checkpoint 4 with ACP-backed risk synthesis")
    ...
    print(json.dumps({
        ...
        "providerJobs": report["execution"]["providerJobs"],
